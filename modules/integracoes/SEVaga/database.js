...
function nextSeq(empresaId) {
  ...
  return (d.integracoes_se_vaga || []).reduce((m, l) => Math.max(m, l._seq || 0), 0) + 1;
}
...
module.exports = {
  getConfig(empresaId) {
    return {
      ...
      se_api_flow_id:     null,
      ...(load(empresaId).se_vaga_config || {}),
    };
  },

  saveConfig(empresaId, cfg) {
    const d = load(empresaId);
    d.se_vaga_config = { ...(d.se_vaga_config || {}), ...cfg };
    persist(empresaId, d);
  },

  saveLog(empresaId, entry) {
    const d = load(empresaId);
    if (!d.integracoes_se_vaga) d.integracoes_se_vaga = [];
    entry._seq = nextSeq(empresaId);
    d.integracoes_se_vaga.unshift(entry);
    persist(empresaId, d);
    return entry;
  },

  listLogs(empresaId, { status, vaga_nome, data_inicio, data_fim, page = 1, limit = 50 } = {}) {
    const d    = load(empresaId);
    let   logs = d.integracoes_se_vaga || [];

    if (status)    logs = logs.filter(l => l.status === status);
    if (vaga_nome) { const q = vaga_nome.toLowerCase(); logs = logs.filter(l => (l.vaga_funcao_nome || '').toLowerCase().includes(q)); }
  ...
  getLogBySeq(empresaId, seq) {
    return (load(empresaId).integracoes_se_vaga || []).find(l => l._seq === Number(seq)) || null;
  },
  ...
  getStatusMapa(empresaId) {
    const logs = load(empresaId).integracoes_se_vaga || [];
    const mapa = {};
    for (const l of [...logs].reverse()) {
      if (!mapa[l.vaga_id]) mapa[l.vaga_id] = l.status;
    }
    return mapa;
  },
  ...
  resetarIntegracao(empresaId, seq) {
    const d    = load(empresaId);
    const logs = d.integracoes_se_vaga || [];
