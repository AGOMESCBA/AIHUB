...
const SQL_BASE = `SELECT
    ...
    SLA_PA_HORAS                    AS sla_horas_pa_chamado,
...
const CAMPOS_SEMANTICOS = [
  ...
  { coluna: 'nome_analista', tipo: 'dimensao', descricao: 'Nome do analista responsável atual pelo chamado.', sinonimos: 'analista, responsavel, atendente, consultor, quem esta atendendo', filtravel: 1, agrupavel: 1, ordenavel: 0, regra: 'Use para "chamados do analista X", "chamados atendidos por X", "chamados por atendente/consultor". Em SoftExpert Chamados, atendente, consultor e analista representam o mesmo responsavel atual.' },
  ...
  { coluna: 'sla_horas_pa_chamado', tipo: 'metrica', descricao: 'Tempo total em horas previsto para o primeiro atendimento do chamado (PA).', sinonimos: 'sla em horas, prazo primeiro atendimento, tempo previsto primeiro atendimento, sla pa, sla primeiro atendimento', filtravel: 1, agrupavel: 0, ordenavel: 1, regra: 'Use para perguntas sobre prazo/tempo previsto do PRIMEIRO ATENDIMENTO. Este campo e previsto/contratado, nao e duracao real nem tempo gasto com uma area.' },
  { coluna: 'sla_padrao_horas_chamado', tipo: 'metrica', descricao: 'Tempo total em horas previsto para atendimento do chamado, ja incluindo o tempo previsto do primeiro atendimento.', sinonimos: 'sla contratado, meta de sla, sla padrao, tempo previsto atendimento, prazo total previsto, prazo contratado atendimento', filtravel: 1, agrupavel: 0, ordenavel: 0, regra: 'Use para perguntas sobre prazo previsto/contratado do atendimento total do chamado. Nao confundir com duracao real do chamado nem com tempos por area.' },
  { coluna: 'sla_situacao_atual_chamado', tipo: 'status', descricao: 'Situação atual do chamado em relação ao SLA: se o chamado está "Em dia" ou "Em atraso".', sinonimos: 'sla em dia, sla atrasado, situacao do sla, esta atrasado, status atual do sla, prazo atual', filtravel: 1, agrupavel: 1, ordenavel: 0, regra: 'Use os valores EXATOS: sla_situacao_atual_chamado = \'Em dia\' ou sla_situacao_atual_chamado = \'Em atraso\'. IMPORTANTE: esta coluna sozinha NAO diferencia chamados abertos de encerrados — \'Em dia\' e \'Em atraso\' tambem aparecem em chamados ja encerrados. REGRA PADRAO: quando o usuario perguntar "chamados no prazo" ou "chamados em atraso" SEM mencionar "encerrado"/"fechado" explicitamente, SEMPRE considere apenas chamados ABERTOS: status_chamado IN (\'Pendente\',\'Andamento\') AND sla_situacao_atual_chamado = \'Em dia\'|\'Em atraso\'. Se o usuario pedir explicitamente "chamados ENCERRADOS em atraso" ou similar, use sla_situacao_final_chamado (nao esta coluna) combinado com status_chamado = \'Encerrado\'.' },
  { coluna: 'sla_situacao_final_chamado', tipo: 'status', descricao: 'Situação final do chamado em relação ao SLA no momento do encerramento: se foi encerrado no prazo ou em atraso.', sinonimos: 'sla cumprido, sla estourado, encerrado no prazo, encerrado em atraso, final do sla, situacao final do sla', filtravel: 1, agrupavel: 1, ordenavel: 0, regra: 'Use os valores EXATOS: sla_situacao_final_chamado = \'No Prazo\' ou sla_situacao_final_chamado = \'Em Atraso\'. Use esta coluna SOMENTE quando o usuario pedir explicitamente por chamados ENCERRADOS/FECHADOS (ex: "chamados encerrados em atraso", "chamados fechados fora do prazo") — nesse caso combine com status_chamado = \'Encerrado\'. Para perguntas genericas "chamados em atraso"/"no prazo" sem mencionar encerramento, use sla_situacao_atual_chamado (nao esta coluna), que por padrao considera so chamados abertos.' },
  ...
  { coluna: 'sla_horas_minutos', tipo: 'texto', descricao: 'SLA formatado como texto no padrão horas:minutos.', sinonimos: 'sla hh:mm', filtravel: 0, agrupavel: 0, ordenavel: 0, regra: 'Formato texto (HH:MM) — não usar em cálculo agregado; usar sla_horas_pa_chamado para métricas de primeiro atendimento, ou sla_padrao_horas_chamado para SLA previsto completo.' },
  { coluna: 'id_status_sla', tipo: 'status', descricao: 'Status operacional do SLA, indicando se o chamado está pausado ou em atendimento.', sinonimos: 'status do sla, sla pausado, chamado pausado, sla em atendimento, pausado, em atendimento', filtravel: 1, agrupavel: 1, ordenavel: 0, regra: 'Use quando a pergunta for sobre SLA pausado, chamado pausado ou chamado em atendimento. Nao use para saber se esta em dia/atraso; nesse caso use sla_situacao_atual_chamado ou sla_situacao_final_chamado conforme aberto/encerrado.' },
  ...
  { coluna: 'dias_duracao_chamado', tipo: 'metrica', descricao: 'Tempo total em dias que o chamado ficou em atendimento.', sinonimos: 'duracao em dias, dias de duracao, tempo total em dias, tempo total do chamado, dias em atendimento', filtravel: 1, agrupavel: 0, ordenavel: 1, regra: 'Use para "quanto tempo demorou", "duracao total", "tempo total em dias" e "chamados com maior duracao". Nao use para perguntas de SLA por area/responsavel.' },
  { coluna: 'horas_duracao_chamado', tipo: 'metrica', descricao: 'Tempo total em horas que o chamado ficou em atendimento.', sinonimos: 'duracao em horas, horas de duracao, tempo total em horas, horas em atendimento', filtravel: 1, agrupavel: 0, ordenavel: 1, regra: 'Use quando o usuario pedir explicitamente duracao/tempo total em horas. Nao use para tempo com cliente, suporte, fabricante ou area especifica.' },
  { coluna: 'dias_duracao_com_suporte', tipo: 'metrica', descricao: 'Tempo total em dias que o chamado ficou em atendimento com o suporte da empresa do IA Command selecionada (ex: J2A ou C3I).', sinonimos: 'dias com suporte, tempo com suporte, tempo com a empresa, tempo com J2A, tempo com C3I, tempo com nossa empresa, suporte da empresa', filtravel: 1, agrupavel: 0, ordenavel: 1, regra: 'Use para perguntas de SLA/tempo em que o chamado ficou com o suporte ou com a empresa do IA Command selecionada. Se a pergunta citar desenvolvimento/FSW, use dias_duracao_com_fsw.' },
  { coluna: 'dias_duracao_com_fsw', tipo: 'metrica', descricao: 'Tempo total em dias que o chamado ficou em atendimento com a area de desenvolvimento da empresa do IA Command selecionada (FSW/Fabrica de Software).', sinonimos: 'dias com FSW, dias com desenvolvimento, dias com fabrica de software, tempo com desenvolvimento, tempo com FSW, tempo com dev', filtravel: 1, agrupavel: 0, ordenavel: 1, regra: 'Use para perguntas de SLA/tempo com desenvolvimento, FSW ou fabrica de software da empresa do IA Command.' },
  { coluna: 'dias_duracao_com_fabricante', tipo: 'metrica', descricao: 'Tempo total em dias que o chamado ficou em atendimento com o fabricante do sistema (ex: Protheus ou SoftExpert).', sinonimos: 'dias com fabricante, tempo com fabricante, tempo com distribuidor, tempo com Protheus, tempo com SoftExpert', filtravel: 1, agrupavel: 0, ordenavel: 1, regra: 'Use para perguntas de SLA/tempo com fabricante, distribuidor, Protheus ou SoftExpert como fabricante do sistema.' },
  { coluna: 'dias_duracao_com_cliente', tipo: 'metrica', descricao: 'Tempo total em dias que o chamado ficou em testes ou aguardando o usuario do cliente (empresa que abriu o chamado).', sinonimos: 'dias com cliente, tempo aguardando cliente, tempo com usuario do cliente, tempo em testes com cliente, retorno do cliente, homologacao do cliente', filtravel: 1, agrupavel: 0, ordenavel: 1, regra: 'Use para perguntas de SLA/tempo com o cliente, usuario do cliente, testes, homologacao ou retorno do cliente. Se citar TI/tecnologia do cliente, use dias_duracao_com_TIcliente.' },
  { coluna: 'dias_duracao_com_TIcliente', tipo: 'metrica', descricao: 'Tempo total em dias que o chamado ficou com a area de tecnologia/TI do cliente (empresa que abriu o chamado).', sinonimos: 'dias com TI do cliente, tempo com TI cliente, tempo com tecnologia do cliente, area de tecnologia do cliente, area de TI do cliente', filtravel: 1, agrupavel: 0, ordenavel: 1, regra: 'Use para perguntas de SLA/tempo com TI do cliente ou area de tecnologia do cliente. Nao confundir com suporte/desenvolvimento da empresa do IA Command.' },
];
...
const FRASES_EXEMPLO = [
  ...
  'tempo medio que o chamado ficou com o cliente', 'tempo com a TI do cliente',
  'tempo com suporte da empresa', 'tempo com desenvolvimento FSW',
  'tempo com fabricante Protheus', 'SLA por area dos chamados',
  'prazo previsto para primeiro atendimento', 'sla padrao em horas',
  'chamados com SLA pausado', 'chamados em atendimento pelo SLA',
].join('\n');
