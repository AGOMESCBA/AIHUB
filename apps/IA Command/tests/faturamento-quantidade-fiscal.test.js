...
assert(systemPrompt.includes("SD2.D2_CF NOT LIKE '59%' AND SD2.D2_CF NOT LIKE '60%'"), 'faturamento financeiro/quantidade faturada deve excluir CF 59/60');
assert(!systemPrompt.includes("SD2.D2_CF NOT LIKE '69%'"), 'regra antiga de exclusao por CF 69 nao deve permanecer no prompt de faturamento');
assert(systemPrompt.includes("Quantidade carregada: SUM(SD2.D2_QUANT), filtrando SD2.D2_CF <> '5117'"), 'quantidade carregada deve excluir CF 5117');
assert(systemPrompt.includes("Entrega futura, venda para entrega futura ou nota mae: SUM(SD2.D2_QUANT), filtrando SD2.D2_CF = '5117'"), 'entrega futura/nota mae deve filtrar CF 5117');
...
WHERE SD2.D_E_L_E_T_ = ' ' AND SF2.D_E_L_E_T_ = ' '
  ...
  AND SD2.D2_CF NOT LIKE '59%'
  AND SD2.D2_CF NOT LIKE '60%'
`;
assert.strictEqual(
  runner._test.validarSqlIaOwnerBasico(sqlQuantidadeFaturada, faturamentoSpec, sx2).ok,
  true,
  'SQL de quantidade faturada com exclusao 59/60 deve ser aceito',
);
...
`;
assert.strictEqual(
  runner._test.validarSqlIaOwnerBasico(sqlQuantidadeCarregada, faturamentoSpec, sx2).ok,
  true,
  'SQL de quantidade carregada excluindo 5117 deve ser aceito',
);
