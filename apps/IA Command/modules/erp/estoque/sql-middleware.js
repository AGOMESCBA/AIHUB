...
const TABELAS_SCHEMA_ESTOQUE = new Set(['SB2', 'SB1', 'SBM', 'SD3']);
...
function processar(sql, cfg = {}) {
  ...
  const foraSchema = [..._extrairTabelas(sql)].filter(t => !TABELAS_SCHEMA_ESTOQUE.has(t));
  if (foraSchema.length) resultado.alertas.push(`Tabelas fora do schema de estoque: ${foraSchema.join(', ')}`);
...
module.exports = { processar, carregarConfig, _extrairTabelas, TABELAS_SCHEMA_ESTOQUE };
