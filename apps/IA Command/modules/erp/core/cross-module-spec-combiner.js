...
- REGRA CRITICA - dono semantico das metricas: em contexto cross-module, cada alias de saida deve nascer do modulo correto. Aliases de faturamento (total_faturamento, faturamento, valor_faturamento, receita) representam nota fiscal de SAIDA e devem usar SEMPRE SD2 JOIN SF2 com SUM(SD2.D2_TOTAL) — NUNCA SF2.F2_VALBRUT (SF2 sozinha nao entra mais no caminho de faturamento; ver DIRETRIZ DE SELECAO DE TABELAS do modulo Faturamento). Aliases de compras (total_compras, compras, valor_compra, valor_compras) representam nota fiscal de ENTRADA e devem usar SD1.D1_TOTAL ou, sem SD1, SF1.F1_VALBRUT.
- REGRA CRITICA - faturamento "considerando devolucao"/"liquido" em contexto cross-module: quando a pergunta pedir faturamento (sozinho ou junto de compras) "considerando devolucoes", "com devolucoes", "abatendo devolucoes" ou "liquido", calcule o alias de faturamento como SUM(SD2.D2_TOTAL - SD2.D2_VALDEV), filtrando SEMPRE SF2.F2_TIPO = 'N' (nota normal) — SD2.D2_VALDEV e o valor de devolucao ja vinculado ao item, dentro da PROPRIA SD2, entao o filtro de tipo de nota continua sendo 'N', igual a qualquer outra consulta de faturamento. PROIBIDO usar SF2.F2_TIPO = 'D' para faturamento liquido — F2_TIPO='D' filtra APENAS notas de devolucao (retorna vazio/zero, pois a maioria das notas e tipo 'N'), nao e o mesmo conceito de "liquido". NUNCA use SF1/SD1 para calcular o faturamento liquido — isso mistura o padrao de "devolucao de venda pura" (abaixo) com o de "faturamento liquido" (SD2.D2_VALDEV), que sao conceitos diferentes.
  EXEMPLO OBRIGATORIO — "faturamento e compras do mes considerando devolucao": COALESCE((SELECT SUM(SD2.D2_TOTAL - SD2.D2_VALDEV) FROM SD2... JOIN SF2... WHERE SF2.F2_TIPO = 'N' AND SF2.F2_EMISSAO BETWEEN '20260701' AND '20260731'), 0) AS faturamento_liquido, COALESCE((SELECT SUM(SD1.D1_TOTAL) FROM SD1... JOIN SF1... WHERE SF1.F1_TIPO IN ('N','C') AND SD1.D1_DTDIGIT BETWEEN '20260701' AND '20260731'), 0) AS total_compras.
- REGRA CRITICA - SF1/SD1 nao sao faturamento normal: SF1 e SD1 pertencem ao caminho de notas de ENTRADA. Em uma consulta de faturamento (bruto ou liquido), e PROIBIDO calcular total_faturamento ou faturamento_liquido com SF1.F1_VALBRUT, SF1.F1_TOTALNF, SF1.F1_VALMERC ou SD1.D1_TOTAL. SF1/SD1 so entram no modulo Faturamento quando a pergunta pedir EXCLUSIVAMENTE devolucao de venda isolada (ex: "total de devolucoes de vendas do mes", sem pedir faturamento junto), usando SF1.F1_TIPO = 'D' e projetando aliases proprios de devolucao (total_devolucoes, valor_devolucao) — nunca como total_faturamento nem faturamento_liquido.
- REGRA CRITICA - comparativo compras x faturamento: para perguntas como "total das compras e do faturamento de junho/2026 comparando com junho/2025", gere metricas independentes por modulo: compras = SD1/SF1 filtrado por SD1.D1_DTDIGIT; faturamento = SD2/SF2 filtrado por SF2.F2_EMISSAO (SUM(SD2.D2_TOTAL), ou SUM(SD2.D2_TOTAL - SD2.D2_VALDEV) se a pergunta pedir liquido/devolucao). Nunca reutilize SF1/SD1 para a metrica de faturamento.
- REGRA CRITICA - comparativo de "esses dois meses" entre compras e faturamento: retorne linhas normalizadas com UNION ALL e colunas tipo, valor e competencia. Exemplo de formato: SELECT 'compras' AS tipo, SUM(SD1.D1_TOTAL) AS valor, SUBSTRING(SD1.D1_DTDIGIT,1,6) AS competencia ... GROUP BY SUBSTRING(SD1.D1_DTDIGIT,1,6) UNION ALL SELECT 'faturamento' AS tipo, SUM(SD2.D2_TOTAL) AS valor, SUBSTRING(SF2.F2_EMISSAO,1,6) AS competencia ... GROUP BY SUBSTRING(SF2.F2_EMISSAO,1,6). PROIBIDO montar SELECT escalar com quatro colunas total_compras_junho/total_compras_julho/total_faturamento_junho/total_faturamento_julho para esse caso, porque dificulta a formatacao comparativa e costuma induzir aliases duplicados.
...
- REGRA CRITICA — JOIN entre tabelas totalizadoras de modulos distintos: PROIBIDO fazer JOIN direto entre tabelas de metricas de modulos diferentes sem chave relacional definida. Exemplo proibido: JOIN SD1 (itens de compra) com SD2/SF2 (itens/cabecalho de faturamento) apenas por periodo — nao existe chave entre elas, o resultado e produto cartesiano com valores multiplicados. Use subqueries escalares independentes no SELECT: (SELECT COALESCE(SUM(SD1.D1_TOTAL),0) FROM SD1 JOIN SF1 ... WHERE ...) AS total_compras, (SELECT COALESCE(SUM(SD2.D2_TOTAL),0) FROM SD2 JOIN SF2 ... WHERE ...) AS total_faturamento.
- EXCECAO PERMITIDA — devolucao de venda ISOLADA (sem faturamento junto) com contrato relacional: JOIN SD1→SF1 e valido SOMENTE quando a pergunta pedir EXCLUSIVAMENTE devolucao de venda (SF1.F1_TIPO='D'), sem pedir faturamento na mesma consulta. Quando a pergunta pedir faturamento "considerando devolucao"/"liquido", NAO use este JOIN — use SD2.D2_VALDEV dentro da propria SD2 (ver REGRA CRITICA de faturamento liquido acima). O JOIN usa obrigatoriamente a chave completa de documento definida em "Contratos Relacionais do Schema Protheus" — filial + doc + serie + fornece/cliente + loja.
