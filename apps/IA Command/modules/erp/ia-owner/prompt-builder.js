...
function buildSystemPrompt(spec = {}, { modeloBaixasReceber, modeloBaixasPagar, mensagem, temAprovacaoPedidoCompra, temNomeAprovador, temSaldoAlcadaAprovador } = {}) {
  return [
    ...
    '  "periodo": {"tipo": string, "dataInicio": "YYYYMMDD|null", "dataFim": "YYYYMMDD|null", "meses": "number[]|null", "anos": "number[]|null", "origem": string, "motivo": string},',
    '  "filtros": object,',
    '  "agrupamentos": string[],',
    ...
    'REGRA WHATSAPP — PROIBIDO incluir uma linha EXTRA de "Total"/"Geral"/"Subtotal" no proprio resultado (ex: via UNION ALL SELECT \'Total\', SUM(...) ou WITH ROLLUP) quando o SELECT ja lista uma linha por item de uma dimensao (produto, cliente, mes, etc.). O formatador de WhatsApp SEMPRE calcula e exibe Subtotal/Total Geral automaticamente somando todas as linhas retornadas — uma linha de total explicita no SQL soma-se de novo a essa soma automatica, duplicando o valor exibido ao usuario. Retorne APENAS as linhas de detalhe (uma por item); nunca uma linha agregada extra representando o todo. Esta regra NAO proibe calcular e retornar outras metricas/colunas por linha (ex: percentual de cada item sobre o total, quando a pergunta pedir) — proibe apenas uma LINHA a mais que representa o agregado geral; toda coluna pedida pela pergunta continua obrigatoria em cada linha de detalhe.',
    ...
    '- REGRA CRITICA - GUIA TECNICO: quando o prompt trouxer "GUIA TECNICO DE SQL", ele traz validacoes e particularidades do Protheus. Use como defesa tecnica, nao como substituto da sua decisao semantica.',
    '',
    ...
    '- CAMPOS periodo.meses e periodo.anos: quando o periodo NAO for um intervalo continuo (mes unico em varios anos, range de meses recorrente em varios anos, meses avulsos em varios anos), preencha periodo.meses (lista de numeros 1-12, ex: [6] ou [1,2,3,4,5,6]) e periodo.anos (lista de numeros, ex: [2025,2026]) alem de dataInicio/dataFim. dataInicio/dataFim continuam obrigatorios sempre, cobrindo do inicio do primeiro periodo ao fim do ultimo (o "envelope" do range total) — meses/anos e que descrevem a granularidade real pedida. Quando o periodo for um intervalo continuo simples (ex: "de janeiro a junho de 2026"), deixe meses e anos como null.',
    '',

    typeof spec.regrasTecnicas === 'function'
      ? spec.regrasTecnicas({ modeloBaixasReceber, modeloBaixasPagar, mensagem, temAprovacaoPedidoCompra, temNomeAprovador, temSaldoAlcadaAprovador })
      : (spec.regrasTecnicas || ''),
  ].filter(Boolean).join('\n');
}
...
function buildUserPrompt({ mensagem, historico, estadoAnterior, contextoTecnico, entidadesResolvidas, tentativa, erroSql, sqlComErro } = {}) {
  ...
  return [
    ...
    queryPlanTexto ? `\n## GUIA TECNICO DE SQL (leia antes de gerar qualquer SQL)\n${queryPlanTexto}\nUse este guia para respeitar particularidades tecnicas do Protheus, seguranca e integridade de dados. A decisao semantica final continua sendo sua: se a pergunta pedir periodo, agrupamento, filtro ou forma diferente do guia, siga a pergunta e gere o melhor SQL valido. Se periodo for explicito, use exatamente as datas declaradas; nao substitua por data_atual, ano atual ou outro ano inferido.` : '',
