...
const TABELAS_SCHEMA_COMPRAS = new Set(['SF1', 'SD1', 'SF2', 'SD2', 'SB1', 'SBM', 'SA2', 'SC7', 'CTT', 'SED', 'SF4']);
...
function _injetarFiltroTenant(sql, cfg) {
  ...
  const posGrupo = sql.search(/\b(?:GROUP\s+BY|ORDER\s+BY|HAVING)\b/i);
  if (posGrupo !== -1) return sql.slice(0, posGrupo) + `WHERE ${filtro}\n` + sql.slice(posGrupo);
  return sql.trimEnd() + `\nWHERE ${filtro}`;
}
...
function processar(sql, cfg = {}) {
  ...
  const foraSchema = [..._extrairTabelas(sql)].filter(t => !TABELAS_SCHEMA_COMPRAS.has(t));
  if (foraSchema.length) resultado.alertas.push(`Tabelas fora do schema de compras: ${foraSchema.join(', ')}`);
...
module.exports = { processar, carregarConfig, _extrairTabelas, TABELAS_SCHEMA_COMPRAS };
