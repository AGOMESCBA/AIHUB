...
function adaptarSqlCanonicoPorSX2(sql, sx2 = {}, opts = {}) {
  ...
  // Corrige placeholders de prompt gerados pela IA, como SB2<010> SB2.
  // O valor entre <...> e apenas exemplo textual; a fonte real do sufixo continua sendo o SX2 atual.
  let texto = String(sql || '').replace(
    /\b(FROM|JOIN)\s+([A-Z][A-Z0-9]{1,3})\s*<\s*[A-Z0-9_]+\s*>([ \t]+(?:AS[ \t]+)?([A-Z_][A-Z0-9_]*))?/gi,
    (match, clausula, tabela, aliasParte, alias) => {
      ...
      const aliasNorm = String(aliasReal || base).toUpperCase();
      const candidatas = tabelaFisicaSX2(sx2, aliasNorm);
      let novaTabela = candidatas.length === 1 ? candidatas[0] : null;
      if (!novaTabela && sufixoFallback) novaTabela = `${base}${sufixoFallback}`;
      if (!novaTabela) return `${clausula} ${base}${aliasParte || ''}`;
      alterou = true;
      avisos.push(`${tabela}<...>${aliasReal ? ` ${aliasNorm}` : ''} -> ${novaTabela}${aliasReal ? ` ${aliasNorm}` : ''}`);
