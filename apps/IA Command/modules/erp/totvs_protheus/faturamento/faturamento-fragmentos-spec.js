...
- REGRA FISCAL BRASILEIRA DE CFOP PARA RECEITA: venda/faturamento/receita significa somente operacoes que geram receita. O "X" dos grupos fiscais representa o primeiro digito do CFOP de saida: 5=saida/prestacao dentro do estado, 6=saida/prestacao fora do estado no Brasil, 7=saida/prestacao para exterior/exportacao. O primeiro digito indica destino/sentido; a natureza de receita, remessa, transferencia, devolucao ou patrimonio vem do grupo/codigo completo.
- CFOP de remessa, transferencia, devolucao de compra, ativo/material de uso ou consumo, credito/ressarcimento de ICMS e devolucao de compra com ST nao representa receita operacional de venda no entendimento fiscal nacional, independentemente do ERP. No Protheus, aplique isso pelo campo SD2.D2_CF: AND (NOT (SD2.D2_CF LIKE '59%' OR SD2.D2_CF LIKE '69%') OR SD2.D2_CF IN ('5932','6932','5933','6933')) AND SD2.D2_CF NOT IN ('5151','6151','5152','6152','5155','6155','5156','6156') AND NOT (SD2.D2_CF LIKE '52%' OR SD2.D2_CF LIKE '62%') AND SD2.D2_CF NOT IN ('5410','6410','5411','6411','5412','6412','5413','6413') AND NOT (SD2.D2_CF LIKE '55%' OR SD2.D2_CF LIKE '65%') AND NOT (SD2.D2_CF LIKE '56%' OR SD2.D2_CF LIKE '66%'). Esta regra tambem vale para faturamento liquido, medias, rankings, comparativos e agrupamentos por cliente/produto/vendedor.
- EXCECAO PERMANENTE DENTRO DO GRUPO 59/69 (nao depende do que o usuario pediu — sempre vale): 5932/6932 (frete cobrado por transportadora) e 5933/6933 (servico tributado por ISS) SAO receita real e NUNCA devem ser excluidos, mesmo comecando com prefixo 59/69. Somente 5931/6931 (retencao de imposto de frete de transportador autonomo, lancamento interno) e 5934/6934 (remessa simbolica de mercadoria em armazem geral) continuam sem receita dentro desse grupo.
- Excecao ao padrao de receita: se o usuario pedir explicitamente "todas as saidas", "movimentacao total", "venda + remessa + transferencia", "incluindo remessas e transferencias", devolucao de compra, ativo imobilizado, material de uso/consumo, credito/ressarcimento de ICMS, saidas patrimoniais ou equivalente, nao aplique a exclusao de CFOP correspondente ao modo fiscal pedido.
...
- Exemplo completo de "faturamento do mes": SELECT COALESCE(SUM(SD2.D2_TOTAL), 0) AS faturamento FROM SD2... JOIN SF2... WHERE SF2.F2_TIPO = 'N' AND (NOT (SD2.D2_CF LIKE '59%' OR SD2.D2_CF LIKE '69%') OR SD2.D2_CF IN ('5932','6932','5933','6933')) AND SD2.D2_CF NOT IN ('5151','6151','5152','6152','5155','6155','5156','6156') AND NOT (SD2.D2_CF LIKE '52%' OR SD2.D2_CF LIKE '62%') AND SD2.D2_CF NOT IN ('5410','6410','5411','6411','5412','6412','5413','6413') AND NOT (SD2.D2_CF LIKE '55%' OR SD2.D2_CF LIKE '65%') AND NOT (SD2.D2_CF LIKE '56%' OR SD2.D2_CF LIKE '66%') AND [demais filtros D_E_L_E_T_/periodo].
...
WITH faturamento AS (
  ...
  FROM SD2xxx SD2
  JOIN SF2xxx SF2 ON SD2.D2_FILIAL = SF2.F2_FILIAL AND SD2.D2_DOC = SF2.F2_DOC AND SD2.D2_SERIE = SF2.F2_SERIE AND SD2.D2_CLIENTE = SF2.F2_CLIENTE AND SD2.D2_LOJA = SF2.F2_LOJA
  WHERE SF2.D_E_L_E_T_ = ' ' AND SD2.D_E_L_E_T_ = ' ' AND SF2.F2_EMISSAO BETWEEN '20260701' AND '20260731' AND SF2.F2_TIPO = 'N' AND (NOT (SD2.D2_CF LIKE '59%' OR SD2.D2_CF LIKE '69%') OR SD2.D2_CF IN ('5932','6932','5933','6933')) AND SD2.D2_CF NOT IN ('5151','6151','5152','6152','5155','6155','5156','6156') AND NOT (SD2.D2_CF LIKE '52%' OR SD2.D2_CF LIKE '62%') AND SD2.D2_CF NOT IN ('5410','6410','5411','6411','5412','6412','5413','6413') AND NOT (SD2.D2_CF LIKE '55%' OR SD2.D2_CF LIKE '65%') AND NOT (SD2.D2_CF LIKE '56%' OR SD2.D2_CF LIKE '66%')
),
carregamento AS (
...
- GRUPOS FISCAIS DE SD2.D2_CF (mutuamente exclusivos, nunca misture, NUNCA omita quando a pergunta cair em um destes casos):
  REMESSA = (LIKE '59%' OR LIKE '69%') | TRANSFERENCIA = IN ('5151','6151','5152','6152','5155','6155','5156','6156') | ENTREGA_FUTURA/NOTA_MAE = IN ('5117','6117')
- DENTRO do grupo REMESSA (59/69), os codigos 5932/6932 (frete de transportadora) e 5933/6933 (servico tributado por ISS) SAO receita real — so 5931/6931 (retencao de imposto de frete de autonomo) e 5934/6934 (remessa simbolica em armazem geral) representam movimentacao documental sem receita. Essa ressalva vale para a EXCLUSAO PADRAO de faturamento/receita (ver regra abaixo); nao se aplica quando o usuario pede remessa explicitamente como conceito de movimentacao — nesse caso, o grupo inteiro 59/69 representa a resposta pedida.
- SAIDAS SEM RECEITA OPERACIONAL PADRAO: DEVOLUCAO_COMPRA = (LIKE '52%' OR LIKE '62%') | DEVOLUCAO_COMPRA_ST = IN ('5410','6410','5411','6411','5412','6412','5413','6413') | ATIVO/USO_CONSUMO = (LIKE '55%' OR LIKE '65%') — ex: 5557/6557 transferencia de material de uso ou consumo | CREDITO/RESSARCIMENTO_ICMS = (LIKE '56%' OR LIKE '66%')
...
- "faturada"/"faturado"/"faturamento"/"vendas"/"receita" SEM mencionar carregada/carga/entrega futura/nota mae/remessa/transferencia/devolucao de compra/ativo/material de uso/credito/ressarcimento/todas as saidas: aplique a regra nacional de receita e exclua saidas sem receita operacional: AND (NOT (SD2.D2_CF LIKE '59%' OR SD2.D2_CF LIKE '69%') OR SD2.D2_CF IN ('5932','6932','5933','6933')) AND SD2.D2_CF NOT IN ('5151','6151','5152','6152','5155','6155','5156','6156') AND NOT (SD2.D2_CF LIKE '52%' OR SD2.D2_CF LIKE '62%') AND SD2.D2_CF NOT IN ('5410','6410','5411','6411','5412','6412','5413','6413') AND NOT (SD2.D2_CF LIKE '55%' OR SD2.D2_CF LIKE '65%') AND NOT (SD2.D2_CF LIKE '56%' OR SD2.D2_CF LIKE '66%').
...
- REGRA DE PREFIXO CFOP: no CFOP do Protheus, prefixo 5 = operacao estadual, prefixo 6 = a MESMA operacao interestadual e prefixo 7 = operacao para exterior/exportacao (ex: 5101/6101/7101 sao venda de producao do estabelecimento variando o destino). Os grupos acima ja tem equivalencia 5xxx/6xxx aplicada quando existe par nacional; CFOP 7xxx deve ser tratado como saida/exportacao e so entra em receita quando o codigo completo indicar venda/exportacao.
...
- Quantidade/valor faturado segue a regra nacional de receita por padrao: exclua CFOPs de remessa, transferencia, devolucao de compra, ativo/material de uso ou consumo, credito/ressarcimento de ICMS e devolucao de compra com ST. Filtros especificos de remessa/transferencia/todas as saidas/saidas patrimoniais/devolucao de compra (quando a pergunta pedir explicitamente): ver regras no bloco de QUANTIDADE/PRODUTO acima — aplicam-se igualmente aqui sempre que SD2 estiver envolvido.
