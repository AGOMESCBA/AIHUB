...
function buildPrompt(mensagem, intencoes = [], sinonimos = [], contextoAnterior = null) {
  ...
  const prompt = [
    ...
    '    "status": string|null',
    '  },',
    '  "agrupar_por": string|null,   // dimensão de agrupamento: "cliente", "produto", "vendedor", "fornecedor", "empresa", "mes", "ano", "dia" — preencher quando o usuário pede breakdown',
    '  "agrupar_por_composto": string[]|null, // duas dimensoes juntas, ex: ["dia","cliente"] quando o usuario pedir "por dia e cliente"',
    ...
    '   - "por dia", "dia a dia", "diário", "detalhe por dia", "diariamente" → agrupar_por = "dia"',
    '   - Sem pedido explícito → agrupar_por = null (retorna total consolidado)',
    '   - Duas dimensoes explicitas juntas: preencha agrupar_por_composto. Ex: "por dia e cliente" → agrupar_por="cliente", agrupar_por_composto=["dia","cliente"]',
    '',
