...
function buildPrompt(mensagem, intencoes = [], sinonimos = [], contextoAnterior = null) {
  ...
  const temDinamicas = intencoes.some(i => String(i.acao || '').toLowerCase() === 'ai_text_to_sql' || String(i.nome || '').toLowerCase().endsWith('_dinamico'));
  const secaoDinamica = temDinamicas ? [
    'Regra financeira obrigatoria: "saldo bancario", "saldos bancarios", "fluxo de caixa", "fluxo de caixa realizado" e "fluxo de caixa projetado" sempre devem ser classificados como financeiro_dinamico.',
    '## Regras para intenções dinâmicas ERP',
    'Quando houver intenções *_dinamico ou acao ai_text_to_sql, escolha o módulo pelo significado do processo, não apenas por uma palavra solta.',
    '',
    '- financeiro_dinamico: contas a pagar/receber, títulos, duplicatas, documentos pagos, documentos recebidos, pagamentos, recebimentos, saldo, vencidos, em aberto.',
    '  * "documentos pagos", "títulos pagos", "duplicatas pagas", "notas pagas" = financeiro_dinamico, carteira pagar, fornecedor/SA2.',
    '  * "documentos recebidos", "títulos recebidos", "duplicatas recebidas" = financeiro_dinamico, carteira receber, cliente/SA1.',
    '- faturamento_dinamico: vendas, faturamento, receita, notas fiscais de saída, NF-e emitidas, clientes/produtos vendidos.',
    '- compras_dinamico: compras, entradas, notas fiscais de entrada, pedidos de compra, ordens de compra, fornecedores/produtos comprados.',
    '- comissao_dinamico: comissão de vendedores, comissão paga/a receber, percentual/base de comissão.',
    '',
    'Se a frase misturar termos genéricos como "documento", "nota" ou "fiscal" com "pago/recebido", priorize o financeiro.',
    'Se a frase disser "nota fiscal de entrada", priorize compras. Se disser "nota fiscal de saída", "venda" ou "faturamento", priorize faturamento.',
    'Agrupamentos válidos incluem: cliente, fornecedor, vendedor, produto, documento, ano, mes, dia, filial, empresa.',
  ].join('\n') : '';
  ...
  const prompt = [
    ...
    '    "status": string|null',
    '  },',
    '  "group_by": string[]|null,    // lista ORDENADA de agrupamentos. Preserve a ordem do texto: ["produto","cliente"] != ["cliente","produto"]',
    '  "agrupar_por": string|null,   // compatibilidade: primeiro item de group_by, ou uma dimensão unica',
    '  "agrupar_por_composto": string[]|null, // compatibilidade: igual a group_by quando houver 2+ dimensoes',
    ...
    '   explicitamente palavras de comparação: "vs", "versus", "contra", "comparar", "comparando",',
    '   "comparativo", "comparado", "crescimento", "com o mesmo período", "ano a ano", "mês a mês".',
    ...
    '      Também se aplica a frases com range de meses explícito ("de [mês_ini] a [mês_fim]") + dois anos + palavra de comparação.',
    ...
    '      Ex: "crescimento de 2025 comparando com 2026 de janeiro a maio" → comparacao_acumulado_mes, mes=5, ano_base=2025, ano_comparacao=2026',
    '      Ex: "faturamento de 2025 vs 2026 de janeiro a março" → comparacao_acumulado_mes, mes=3, ano_base=2025, ano_comparacao=2026',
    ...
    '',
    '6. AGRUPAMENTO: preencha "group_by" quando o usuário pedir breakdown por uma ou mais dimensões.',
    '   REGRA CRÍTICA: preserve a ordem exata das dimensões na frase do usuário.',
    '   O primeiro item de group_by é o agrupamento principal; o segundo é detalhamento dentro do primeiro.',
    '   Para compatibilidade, agrupar_por deve receber sempre o primeiro item de group_by.',
    ...
    '   - "por fornecedor", "ranking de fornecedores", "top fornecedores" → group_by=["fornecedor"], agrupar_por="fornecedor"',
    '   - "por empresa" -> group_by=["empresa"], agrupar_por="empresa" (empresa do IAHub/canal)',
    '   - "por filial" / "por loja" -> group_by=["filial"], agrupar_por="filial" (coluna do dataset)',
    '   - "por unidade" / "por unidade de negocio" -> group_by=["unidade"], agrupar_por="unidade" (coluna do dataset)',
    '   - "por mês", "mês com maior faturamento", "melhor mês", "ranking mensal" → group_by=["mes"], agrupar_por="mes"',
    '   - "por ano", "ano com maior faturamento", "melhor ano", "ranking anual" → group_by=["ano"], agrupar_por="ano"',
    '   - "por dia", "dia a dia", "diário", "detalhe por dia", "diariamente" → group_by=["dia"], agrupar_por="dia"',
    '   - Sem pedido explícito → group_by=null, agrupar_por=null (retorna total consolidado)',
    '   - Duas ou mais dimensoes explicitas juntas: use group_by com a mesma ordem do texto.',
    '   - "empresa" significa empresa do IAHub/canal. Para dados internos do dataset, prefira "filial" ou "unidade".',
    '   - "por filial" / "por loja" -> group_by=["filial"], agrupar_por="filial".',
    '   - "por unidade" / "por unidade de negocio" -> group_by=["unidade"], agrupar_por="unidade".',
    '   - "todos os meses" e "cada mes" tambem sao agrupamento por mes, nao periodo mes_atual.',
    '   - "faturamento por ano e todos os meses por cliente e produto" -> periodo.tipo="nenhum", group_by=["ano","mes","cliente","produto"], agrupar_por="ano", agrupar_por_composto=["ano","mes","cliente","produto"]',
    '   - "vendas por produto e cliente do ano passado" → group_by=["produto","cliente"], agrupar_por="produto", agrupar_por_composto=["produto","cliente"]',
    '   - "vendas por cliente e produto do ano passado" → group_by=["cliente","produto"], agrupar_por="cliente", agrupar_por_composto=["cliente","produto"]',
    '   - "por dia e cliente" → group_by=["dia","cliente"], agrupar_por="dia", agrupar_por_composto=["dia","cliente"]',
    '   - "agrupando por mes e detalhando por fornecedor" → group_by=["mes","fornecedor"], agrupar_por="mes", agrupar_por_composto=["mes","fornecedor"]',
    '   - "agrupando por mes e detalhando por cliente" → group_by=["mes","cliente"], agrupar_por="mes", agrupar_por_composto=["mes","cliente"]',
    '   - "agrupando por mes e detalhando por produto" → group_by=["mes","produto"], agrupar_por="mes", agrupar_por_composto=["mes","produto"]',
    '   - "agrupando por fornecedor e detalhando por produto" → group_by=["fornecedor","produto"], agrupar_por="fornecedor", agrupar_por_composto=["fornecedor","produto"]',
    '   REGRA: "agrupando por X e detalhando por Y" = group_by=["X","Y"]. "detalhando por Y" é SEMPRE um segundo nível de agrupamento, nunca um filtro.',
    '',
